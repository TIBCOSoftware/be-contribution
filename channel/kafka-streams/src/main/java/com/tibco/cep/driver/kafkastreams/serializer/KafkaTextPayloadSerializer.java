 ...
 * @author moshaikh
 */
public class KafkaTextPayloadSerializer extends BaseEventSerializer implements KafkaSerializer {
	...
	public Object serializeUserEvent(EventWithId event, Map<String, Object> properties) throws Exception {
		...
		key = key == null ? event.getExtId() : String.valueOf(key);
		...
		ProducerRecord kafkaMessage = new ProducerRecord(null == replyTopic ? topic : replyTopic, key, ((ExtendedDefaultEventImpl)event).getUnderlyingSimpleEvent().getPayloadAsString());
		return kafkaMessage;
	}
