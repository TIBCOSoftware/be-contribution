...
public class AggregatesBuilder {
	...
	public static List<Bson> build(String aggFunction, List<String> groupByColSet, String aggCol,
			List<Bson> aggegationpipeline) {
		switch (aggFunction) {
		case "SUM":
	...
	// If groupby column has multiple values then those are merged in single
	// document using following function
	...
	// This function builds aggregation pipeline based on groupby column,aggregation
	// column and aggregation function
	public static void buildAggregationPipeline(BsonField aggfunction, List<String> groupByColSet,
			List<Bson> aggegationpipeline) {
		if (groupByColSet.isEmpty()) {
			aggegationpipeline.add(group(null, aggfunction));
			if (aggfunction.getName().equalsIgnoreCase("count")) {
				aggegationpipeline.add(
						project(fields(excludeId(), computed(aggfunction.getName(), "$" + aggfunction.getName()))));
			} else
				aggegationpipeline
						.add(project(fields(excludeId(), computed(aggfunction.getName(), aggfunction.getValue()))));
		} else {
